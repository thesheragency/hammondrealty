...
## Overview
The frontend now implements the **Blake Hammond Real Estate** design: 13 static marketing pages (`components/pages/*Page.tsx`, client components) with shared site chrome in `components/site/` (SiteHeader, SiteFooter, CtaSection, FaqsSection, TestimonialsSection, GoogleBadges). Static app routes (`app/buyer`, `app/seller`, `app/connect`, etc. — slugs match WordPress) are async server wrappers that provide SEO metadata via `buildMetadata`, fetch that page's ACF field group from WordPress via `lib/wp-acf.ts`, and pass it as an `acf` prop to the page component. Page components render ACF values with hardcoded fallbacks, so every page renders identically if WordPress is unreachable or a field is empty. `lib/wp-acf.ts` (generated from WPGraphQL introspection) uses direct `fetch` with `next: { revalidate: 60, tags: ['wp-content'] }` — never graphql-request — because Next's data cache would otherwise serve stale ACF data; it also exposes `fetchFaqs()`/`fetchTestimonials()` for the faq/testimonial custom post types and `imgUrl`/`imgAlt` helpers. Multiline ACF headings (values contain `\r\n`) render inside `whitespace-pre-line` spans. WordPress-driven routes (`app/blog`, `app/[slug]`) use `components/layout/Layout.tsx`, which renders the same SiteHeader/SiteFooter. Design tokens (sage-green brand `95 12% 62%`, Inter + Playfair Display via next/font, elevate interaction system, `lg` breakpoint raised to 1280px) live in `app/globals.css` + `tailwind.config.ts`; `/style-guide` is the visual source of truth. Static images are served from `public/images/`.
...
-   **Suspense Boundaries for Async Server Components:** All async server components (`YoastSchema`, `PostListServer`, `GlobalHeadScripts`/`GlobalBodyScripts`) MUST be wrapped in `<Suspense>` boundaries when rendered in page routes or `app/layout.tsx`. This prevents hydration mismatches in Next.js 16 + Turbopack. Any new async server component must follow this pattern.
-   **Timezone-Stable Date Formatting:** All date formatting that runs on both server and client (e.g., `toLocaleDateString` in `PostCard`, `PostContent`) MUST pass `timeZone: 'UTC'` to ensure server (UTC) and client (user's local timezone) produce identical output. Without this, dates near midnight format differently between SSR and client render, causing hydration failures and "Invalid hook call" cascade errors.
...
-   **301 Redirects:** `proxy.ts` (Next.js 16 proxy convention, replaces the older `middleware.ts` convention) fetches and applies 301/302 redirects from WordPress Headless Tools plugin in real-time, with in-memory caching (5-minute TTL). Any redirect targets stored in WordPress that reference `localhost` are automatically rewritten to `FRONTEND_URL` so external visitors receive a valid destination.
-   **On-Demand Revalidation:** A `/api/revalidate` endpoint allows for purging the Next.js page cache when WordPress content is updated, supporting specific paths, content types, or the entire site. A diagnostic endpoint at `/api/revalidate/ping` (no secret required) confirms connectivity and env-var configuration — use this URL in the WordPress plugin's connectivity test. **WordPress plugin setup:** In the WordPress headless plugin settings, set *Frontend URL* to `https://blakehammondrealty.com` (production) and *Preview Secret* to match the `WP_PREVIEW_SECRET` Replit secret. The revalidation endpoint the plugin should call is `https://blakehammondrealty.com/api/revalidate`. Never use `localhost:5000` as the Frontend URL — WordPress runs on a separate server and cannot reach localhost on the Replit container.
-   **ACF Global Scripts:** Supports injecting custom `<head>` and `<body>` scripts configured through Advanced Custom Fields (ACF) in WordPress. Fetched directly from WordPress GraphQL. The ACF Options Page query field name follows WPGraphQL's title-to-field convention, preserving consecutive uppercase characters as an acronym (e.g., page title "SHER Options" becomes the GraphQL field `sHEROptions`, not `sherOptions`). When changing the ACF Options page name in WordPress, update `GET_ACF_OPTIONS_QUERY` and `WpAcfOptions` in `lib/wordpress.ts` accordingly.
...
-   `proxy.ts` — 301/302 redirect handling (Next.js 16 proxy convention)
