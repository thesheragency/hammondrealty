...
`;
  ...
  // ============================================
  // Gravity Forms API Proxy
  // ============================================
  
  // Get form structure (requires authentication for reading form details)
  app.get('/api/gravity-forms/:formId', async (req: Request, res: Response) => {
    ...
    if (!wpBaseUrl) {
      return res.status(500).json({ 
        ...
        message: 'WP_API_URL environment variable is required'
      });
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({ 
          ...
          message: response.status === 401 ? 'Authentication required' : errorText
        });
      }
  ...
  // Submit form entry (no authentication required for submissions)
  app.post('/api/gravity-forms/:formId/submit', async (req: Request, res: Response) => {
    const { formId } = req.params;
    const wpBaseUrl = getWordPressBaseUrl();
    ...
    try {
      ...
      // Check if response is HTML (WordPress error page) instead of JSON
      if (responseText.trim().startsWith('<')) {
        console.error('Gravity Forms returned HTML instead of JSON. This usually means:');
        console.error('1. The Gravity Forms REST API is not enabled');
        console.error('2. The form ID does not exist');
        console.error('3. The WordPress site is not accessible');
        console.error('Response preview:', responseText.substring(0, 200));
        return res.status(502).json({
          error: 'Invalid response from WordPress',
          message: 'The Gravity Forms REST API returned an HTML error page. Please verify the form ID and that the Gravity Forms REST API is enabled.',
          is_valid: false
        });
      }
      ...
      try {
        ...
        // Forward the Gravity Forms response
        res.status(response.ok ? 200 : 400).json(result);
      } catch (parseError) {
        console.error('Failed to parse Gravity Forms response:', responseText.substring(0, 500));
        res.status(502).json({
          error: 'Invalid JSON response',
          message: 'Failed to parse response from Gravity Forms API',
          is_valid: false
        });
      }
    } catch (error) {
  ...
  // List all forms (requires authentication)
  app.get('/api/gravity-forms', async (req: Request, res: Response) => {
    const wpBaseUrl = getWordPressBaseUrl();
    ...
    try {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ 
          ...
          message: response.status === 401 ? 'Authentication required' : await response.text()
        });
      }
      ...
      console.error('Error fetching Gravity Forms list:', error);
      res.status(500).json({ 
  ...
  const STATIC_FRONTEND_ROUTES = ['/', '/blog', '/contact'];
